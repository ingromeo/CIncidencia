...
class Catinc {
    ...
    function insertarcatinc($nombres, $estado) {
        $rpta;
        try {
            ...
            $sql = "INSERT INTO categoriaincidencia (descripcion_catinc, estado_catinc)
    ...
    function actualizarcatinc($idi, $nombres, $estado) {
        $rpta;
        try {
            //Creamos un objeto de la clase conexion
            //$miconexion = new Conexion();
            ...
            mysql_query("BEGIN");
            //Elaboramos la sentencia
            $sql = "UPDATE categoriaincidencia SET descripcion_catinc = '$nombres',
                                         estado_catinc = '$estado'
                                         WHERE id_catinc = $idi";
            //Ejecutamos la sentencia
            $result = mysql_query($sql);
            $rpta;
    ...
    function eliminarcatinc($idi, $estado) {
        $rpta;
        try {
            //Creamos un objeto de la clase conexion
            //$miconexion = new Conexion();
            ...
            mysql_query("BEGIN");
            //Elaboramos la sentencia
            $sql = "UPDATE categoriaincidencia SET estado_catinc = '$estado'
                                         WHERE id_catinc = $idi";
            //Ejecutamos la sentencia
            $result = mysql_query($sql);
            $rpta;
    ...
    function buscarcatinc($id) {
        //Le deciamos que la locacion es lenguaje espa�ol
        setlocale(LC_CTYPE, 'es');
        //La sentencia a ejecutar
        $sql = "SELECT * FROM categoriaincidencia WHERE id_catinc=$id";
        try {
            //Creamos un objeto de la clase conexion
            //$miconexion = new Conexion();
            //Obtenemos la conexion
