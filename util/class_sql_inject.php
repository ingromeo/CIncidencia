...
class sql_inject
{
	...
	{ 
		// This Will be the fuction we call to protect the variables.
		$protected = strtoupper($protected);

		$banlist = array ("DOCUMENT","WRITE","EVAL","%","UNESCAPE","STRING","FROMCHARCODE","WHERE", "PASS", "BETWEEN", "NULL","TABLE","EXEC", "CREATE", "DROP", "ALTER", "INSERT", "SELECT", "UPDATE", "DELETE", "DISTINC", "HAVING", "TRUNCATE", "REPLACE", "HANDLER", "LIKE", "PROCEDURE", "LIMIT", "ORDER BY", "GROUP BY", "ASC ", "DESC ","skipfish","SKIPFISH","#","TRUE","true","FALSE","false"); 
		//$banlist is the list of words you dont want to allow.
		if ( eregi ( "[a-zA-Z0-9@]+", $protected ) ) 
		{ 
			// Makes sure only legitimate Characters are used.
			$protected = trim(str_replace($banlist, '', $protected)); // Takes out whitespace, and removes any banned words.
			return strtolower($protected);
			//echo "+";
		} 
		else 
		{
